...
class Keyboard(QtWidgets.QGridLayout): # Inherit from QGRidLayout
    ...
    # Deletes one character (to be worked out)
    def Backspace(self):
...
class savedNote:
    ...
    # Delete the QAction if the button isn't needed any more (e.g. replaced by a more recent Saved Note button - see the SaveNotes function)
    def delete(self):
    ...
    def clicked(self, a):
        with open(self.path, "r") as f:
            #Preloading pages
            pageCount = f.read().count(control) - len(self.parent.pages) # Checks if we even need new pages at all
            #if pageCount >= 0:
                #for i in range(len(self.parent.pages)): # TODO make it go to the end before adding new pages
                    #nextPage(self.parent())
        with open(self.path, "r") as f: #Can't do two reads, so I'm bodging
            for i, c in enumerate(f.read().split(control)): # Loop used for page splitting
...
class MainWindow(QtWidgets.QMainWindow): # Inherits goodies from QMainWIndow
    def __init__(self):
        ...
        # Centers the big widget with the text box and handwriting area / keyboard
